...
const TransfereeForm = () => {
    ...
    const handleChange = (e) => {
        ...
        if (/^\s/.test(value)) {
            ...
            setInputData({ ...inputData, [name]: '' });
        } else if (name === 'dob' && new Date(value).getFullYear() <= 1940) {
            Swal.fire({
                icon: 'error',
                title: 'Invalid Date of Birth',
                text: 'Year must be above 1940.',
                confirmButtonColor: '#22C55E',
            });
            setInputData({ ...inputData, [name]: '' });
        } else if (name === 'dob' && (new Date(capitalizedValue) >= new Date())) {
            Swal.fire({
                icon: 'error',
                title: 'Invalid Date of Birth',
            ...
            setInputData({ ...inputData, [name]: '' });
        } else if ((name === 'phone' || name === 'parentPhone') && (/[^0-9]/.test(value) || value.length > 10)) {
            Swal.fire({
                icon: 'error',
                title: 'Invalid Phone Number',
                text: 'Phone number should be a numeric value and exactly 10 digits.',
                confirmButtonColor: '#22C55E',
            });
            setInputData({ ...inputData, [name]: '' });
    ...
    const handleFileChange = (e) => {
        ...
        if (Object.values(inputFile).some(file => file && file.name === selectedFile.name)) {
            Swal.fire({
                icon: 'error',
                title: 'Duplicate File',
                text: 'This file has already been uploaded. Please select a different file.',
                confirmButtonColor: '#22C55E',
            });
            e.target.value = null;
            return;
    ...
    <div className='w-full min-h-screen bg-gradient-to-r from-green-900 to-green-700'>
        ...
        <div className='p-5 w-full'>
            <form ref={form} onSubmit={handleSubmit} className='bg-white max-w-5xl p-12 shadow rounded-lg mt-16 mx-auto max-sm:px-5'>
                <h2 className='text-3xl uppercase font-bold mb-5 text-black max-sm:text-xl'>APPLICATION FORM FOR THE TRANSFEREES</h2>    
                ...
                <div className='flex items-center justify-center gap-[30px] mb-8 max-sm:flex-col'>
                    ...
                    <div className="w-1/3 max-sm:w-full">
                        ...
                        <input className='w-full p-4 pr-4 text-sm text-black rounded-md border-2 border-solid border-gray-200 outline-none bg-[#EBEBEB] placeholder-gray-500' type="text" placeholder='Citizenship' name='citizenship' value={citizenship} onChange={handleChange} required/>
                    </div>
                </div>
                ...
                <h2 className='text-xl font-bold text-black break-all max-sm:text-lg'>Requirements for Transferee Students for the Upper Primary Education</h2>
